 ...
 * Default constructor.
 */
ClipboardJS::ClipboardJS(const std::string& id) :
		m_id(id) {
  m_clipboardmgr = new webworks::ClipboardNDK(this);
}
 ...
 */
string ClipboardJS::InvokeMethod(const string& command) {
	...
	if (strCommand == "setText") {
		return m_clipboardmgr->setText(arg);
	} else if (strCommand == "getText") {
		return m_clipboardmgr->getText();
	}
...
void ClipboardJS::NotifyEvent(const std::string& event) {
